        ...
        <div className="flex">
          ...
          <div className="ml-3">
            <p className="text-xs sm:text-sm text-yellow-700 dark:text-yellow-200">
              No services in inventory yet. Add your first service to get started!
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="overflow-x-auto -mx-3 sm:mx-0 shadow border-b rounded-lg border-gray-200 dark:border-gray-700">
      <table className="min-w-full divide-y text-xs sm:text-sm divide-gray-200 dark:divide-gray-700">
        <thead className="bg-gray-50 dark:bg-gray-700">
          <tr>
            <th scope="col" className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Brand/Name</th>
            ...
            <th scope="col" className="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Description</th>
        ...
        <tbody className="divide-y bg-white dark:bg-gray-800 divide-gray-200 dark:divide-gray-700">
          {services.map((service, index) => (
            <tr key={service.id} className={`${index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-700'} hover:bg-gray-100 dark:hover:bg-gray-600`}>
              ...
              <td className="px-2 sm:px-3 py-2 sm:py-3 text-xs sm:text-sm text-gray-900 dark:text-white text-left max-w-xs truncate" title={service.serviceDescription}>
                {service.serviceDescription || 'N/A'}
              </td>
              <td className="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap text-xs sm:text-sm text-gray-900 dark:text-white text-left">{service.category || 'N/A'}</td>
              <td className="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap text-xs sm:text-sm text-gray-900 dark:text-white text-left">
                {service.purchasePrice ? `₹${parseFloat(service.purchasePrice).toFixed(2)}` : 'N/A'}
              </td>
              <td className="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap text-xs sm:text-sm text-gray-900 dark:text-white text-left">
                {service.salePrice ? `₹${parseFloat(service.salePrice).toFixed(2)}` : 'N/A'}
              </td>
              <td className="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap text-xs sm:text-sm text-gray-900 dark:text-white text-left">
                {parseFloat(service.qty) || 1} units
              </td>
