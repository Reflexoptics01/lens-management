  ...
  const generateReturnNumber = async () => {
    try {
      ...
      if (!financialYear) {
        ...
        const newReturnNumber = `PR-${(snapshot.docs.length + 1).toString().padStart(3, '0')}`;
        return newReturnNumber;
      }
      
      // Get or create the user-specific counter document for purchase returns in this financial year
      const counterRef = getUserDoc('counters', `purchaseReturns_${financialYear}`);
      ...
      if (counter.format) {
        returnNumber = counter.format
          .replace('${prefix}', counter.prefix || 'PR')
          .replace('${separator}', counter.separator || '-')
          .replace('${number}', paddedNumber);
      } else {
        returnNumber = `${counter.prefix || 'PR'}${counter.separator || '-'}${paddedNumber}`;
      }
      ...
      try {
        const returnsRef = getUserCollection('purchaseReturns');
        const snapshot = await getDocs(returnsRef);
        const fallbackNumber = `PR-${(snapshot.docs.length + 1).toString().padStart(3, '0')}`;
    ...
    try {
      ...
      const returnsList = snapshot.docs
        ...
        .map((doc) => ({
          ...
          displayId: doc.data().returnNumber || `PR-${doc.id.slice(-3)}`,
          ...doc.data()
        }));
